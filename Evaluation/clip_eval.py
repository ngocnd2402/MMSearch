...
FEATURES_PATH = "/mmlabworkspace/Students/visedit/AIC2023/Evaluation/Flickr8K/Cvecs"

clip_text_embedd = ClipTextEmbedding()
...
class CLIPRetrieval:
    ...
    def precision_at_k(self, retrieved, relevant, k=1):
        top_k = retrieved[:k]
        relevant_in_top_k = [item for item in top_k if item in relevant]
        return len(relevant_in_top_k) / k

    def recall_at_k(self, retrieved, relevant, k=1):
        top_k = retrieved[:k]
        relevant_in_top_k = [item for item in top_k if item in relevant]
        return len(relevant_in_top_k) / len(relevant)
    
    def average_precision_at_k(self, retrieved, relevant, k):
        ap = 0
        for i in range(1, k+1):
            if retrieved[i-1] in relevant:
                ap += self.precision_at_k(retrieved, relevant, i)
    ...
    def evaluate(self, vector_search_engine, batch_size=64):
        ...
        for batch in tqdm(to_batches(self.gt_data, batch_size), desc="Evaluating & Calculating MAP"):
            for item in batch:
                ...
                precision = self.precision_at_k(retrieved_images, relevant_images, topk_retrieval)
                recall = self.recall_at_k(retrieved_images, relevant_images, topk_retrieval)
                ap = self.average_precision_at_k(retrieved_images, relevant_images, topk_retrieval)
...
clip_retrieval = CLIPRetrieval(GT_PATH, clip_text_embedd)
avg_precision, avg_recall, avg_mrr, map_score = clip_retrieval.evaluate(vector_search_engine)
